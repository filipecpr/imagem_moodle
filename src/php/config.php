...
$CFG->dbuser    = env('CFG_DBUSER', env('POSTGRES_USER', 'ava_user'));
...
if ( (in_array('REQUEST_SCHEME', $_SERVER)) && ("{$_SERVER['REQUEST_SCHEME']}://{$_SERVER['HTTP_HOST']}" == env('CFG_WWWROOT_DOCKER'))) {
...
$CFG->dataroot  = env('CFG_DATAROOT', '/var/moodledata');
...
$CFG->directorypermissions = env_as_int('CFG_DIRECTORYPERMISSIONS', 02777);
...
if (env('CFG_ENV', 'local') == 'local' && env_as_bool('CFG_DEBUG', false)) {
  ...
  @ini_set('display_errors', env('CFG_DISPLAY_ERRORS', '1'));
